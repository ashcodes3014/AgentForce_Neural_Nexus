...
class LLMService:
    ...
    def generate_suggestions(self, resume_summary: str, job_desc: str, matching_issues: list[str]) -> list[str]:
        prompt = PromptTemplate(
            template="Based on the resume summary and job description, provide 3-5 actionable suggestions to improve the resume.\n\nResume Summary:\n{resume_summary}\n\nJob Description:\n{job_desc}\n\nIdentified Matching Issues:\n{matching_issues}\n\nSuggestions:",
    ...
    def generate_cover_letter(self, resume_summary: str, job_title: str, job_desc: str) -> str:
        prompt = PromptTemplate(
            template="Write a professional cover letter for a candidate with the following resume summary applying for a {job_title} position.\n\nJob Description:\n{job_desc}\n\nResume Summary:\n{resume_summary}\n\nCover Letter:",
